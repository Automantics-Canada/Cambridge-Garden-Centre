  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      const [resInvoices, resSuppliers] = await Promise.all([
        api.get('/invoice'), // In real app, would have a /dashboard endpoint
        api.get('/supplier')
      ]);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight flex items-center gap-2">
            Operations Overview <span className="text-2xl">📊</span>
          </h1>
          <p className="text-gray-500 text-sm mt-1">Real-time insights for Cambridge Garden Centre</p>
        </div>
