...
export default function Login() {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    if (email && password) {
      dispatch(login({ name: 'Sarah K.', email, role: 'Admin' }));
      navigate('/dashboard');
    }
  };
    ...
    <div className="min-h-screen bg-[#1D5333] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#1D5333] hover:bg-[#153e26] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#1D5333]"
          >
            Sign In
          </button>
        </form>
